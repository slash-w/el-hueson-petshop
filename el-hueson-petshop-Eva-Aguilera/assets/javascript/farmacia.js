...
fetch('https://mindhub-xj03.onrender.com/api/petshop')
  .then(response => response.json())
  .then(data => {
    // Mostrar solo los primeros 7 elementos
    const primeros7Productos = data.slice(0, 7);
    imprimirCards(primeros7Productos, contenedor);

    // Filtrar por la categoría "farmacia"
    const productosFarmacia = data.filter(product => product.categoria === "farmacia");

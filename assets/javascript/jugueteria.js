...
function imprimirCards(cards, container) {
  ...
  function filtrarProductos(event) {
    ...
    fetch("https://mindhub-xj03.onrender.com/api/petshop")
      ...
      .then((data) => {
        let juguetes = data.filter(e=>e.categoria == "jugueteria")
  
        const productosFiltrados = juguetes.filter((product) => {
  ...
  fetch("https://mindhub-xj03.onrender.com/api/petshop")
    .then((response) => response.json())
    .then((data) => {
      const productosJugueteria = data.filter(
        (product) => product.categoria === "jugueteria"
      );
      console.log(productosJugueteria);
  
      imprimirCards(productosJugueteria, contenedor);
  
      const filteredProducts = productosJugueteria.filter(

...
function imprimirCards(cards, container) {
  ...
  function filtrarProductos(event) {
    ...
    fetch("https://mindhub-xj03.onrender.com/api/petshop")
      ...
      .then((data) => {
        let juguetes = data.filter(e=>e.categoria == "farmacia")
  ...
  fetch("https://mindhub-xj03.onrender.com/api/petshop")
    .then((response) => response.json())
    .then((data) => {
      const productosfarmacia = data.filter(
        (product) => product.categoria === "farmacia"
      );
      console.log(productosfarmacia);
  
      imprimirCards(productosfarmacia, contenedor);
  
      const filteredProducts = productosfarmacia.filter(
